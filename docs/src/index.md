...
`ElectronCall.jl` is loaded internally by PlotlySupply when needed (for `display`, `show=true`, `to_syncplot`, and image/PDF `savefig`), so you do not need `using ElectronCall`.
